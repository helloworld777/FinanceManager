...
import com.lu.momeymanager.view.activity.angle.AngleActivity;
import com.lu.momeymanager.view.activity.angle.AngleFont;
import com.lu.momeymanager.view.activity.angle.AngleObject;
import com.lu.momeymanager.view.activity.angle.AngleSound;
...
import com.lu.momeymanager.view.activity.angle.AngleString;
import com.lu.momeymanager.view.activity.angle.AngleUI;
import com.lu.momeymanager.view.activity.angle.AngleVector;
...
public class GameUI extends AngleUI
{
	...
	public GameUI(AngleActivity activity)
	{
		...
		slGround=new AngleSpriteLayout(mActivity.mGLSurfaceView,R.drawable.fondo);
		slSmiley=new AngleSpriteLayout(mActivity.mGLSurfaceView,32,45, R.drawable.salto,0,0,32,45,8,8);
		slSight=new AngleSpriteLayout(mActivity.mGLSurfaceView,32,32,R.drawable.mira,0,0,32,32,2,2);

		//STEP 17:
		//Create and load a sound effect		
		//>PASO 17:
		//>Creamos y cargamos un efecto de sonido
		sndMachineGun=new AngleSound(mActivity,R.raw.machinegun);
		...
		//>PASO 19:
		//>Creaci�n de los objetos de juego
		//>Aqu� usamos la forma de inserci�n r�pida
		...
		//>y un grupo para marcadores y dem�s cosas flotantes
		ogDashboard=addObject(new AngleObject());
		sprTrackPad=(AngleSprite) ogDashboard.addObject(new AngleSprite(160,380,new AngleSpriteLayout(mActivity.mGLSurfaceView,320,200,R.drawable.panel)));
	...
	//the engine itself.
	//>Paso 22:
	//>Creamos una funci�n para la l�gica del juego.
	//>Esta funci�n ser� llamada cada vez que se pinte la pantalla y recibe como par�metro
	//>la cantidad de segundos que han pasado desde la �ltima vez que se la llam�.
	//>Al llamar a super.step, hacemos que se llame a la funci�n step de todos los hijos que 
	//>tenga este objeto por debajo. En este caso, el campo de batalla, la mirilla y los marcadores
	//>En este paso a paso, no voy a comentar la l�gica del juego. S�lo las partes que correspondan 
	//>al motor en si.
	@Override
	...
	//>Tambi�n podemos sobrecargar otros eventos como onKeyDown y onTrackballEvent para 
	//>responder a distintos m�todos de entrada.
	@Override
	public boolean onTouchEvent(MotionEvent event)
	{
		//Prevent event flooding
		//>Impedimos que el sistema sea sobrecargado por eventos de touch
		//>M�ximo 60 por segundo
		try
		{
			Thread.sleep(16);
		...
		else if (event.getAction()==MotionEvent.ACTION_UP)
		{
			...
			//>PASO 25:
			//>Detenemos la reproducci�n del sonido
			sndMachineGun.stop();
			mSight.fire(false);
